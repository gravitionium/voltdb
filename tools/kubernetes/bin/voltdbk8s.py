...
if __name__ == "__main__":
    ...
    ###TEMPORARY### TODO: remove this tarball for production
    os.system("ls -l " + PV_VOLTDBROOT)
    if not DO_INIT:
        printdbg("%s %s %s" % (PV_VOLTDBROOT, my_hostname, time()))
        os.system("tar --exclude *.tgz -cvzf /%s/%s_%s.tgz ../" % (PV_VOLTDBROOT, my_hostname, time()))
